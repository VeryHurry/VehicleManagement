...
-  (void)drawDashLines {
    ...
    if (XAxisIndexs.count >0 && self.allDates.count > 0) {
        for (int i = 0; i<XAxisIndexs.count; i++) {
            ...
            if (k == 0) {
                xLineView.center = CGPointMake(START_X_VALUE, vHeight-20+3/2);
            }
            else
            {
            ...
            if (k == 0) {
                xAxisLabel.center = CGPointMake(START_X_VALUE, vHeight-xAxisLabel.bounds.size.height/2);
            }
            else
            {
...
- (void)drawDots {
    ...
    for (int i =0; i<self.points.count; i++) {
        ...
        if (isnan(point.x)) {
            dot.center = CGPointMake(0, point.y);
        }
        else
        {
